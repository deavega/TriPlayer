...
namespace Frame {
    AlbumInfo::AlbumInfo(Main::Application * a, AlbumID id) : Frame(a) {
        ...
        this->heading->setString("Album.Information.Heading"_lang);

        // Name
        Aether::Text * txt = new Aether::Text(this->heading->x(), this->heading->y() + this->heading->h() + 20, "Album.Information.Name"_lang, 30);
        ...
        this->name->setKeyboardHint("Album.Information.Name"_lang);
        ...
        // Image heading
        txt = new Aether::Text(this->heading->x(), box->y() + box->h() + 20, "Album.Information.Image"_lang, 30);
        txt->setColour(this->app->theme()->FG());
        this->bottomContainer->addElement(txt);
        ...
        // Save button
        this->saveButton = new Aether::FilledButton(button->x() - this->w()*0.01 - 75, button->y() + button->h() + 25, 150, 50, "Common.Save"_lang, 26, [this]() {
            this->saveChanges();
        });
    ...
    void AlbumInfo::createAudioDBOverlay() {
        ...
        tips->setString("Album.Information.DownloadMessage"_lang);
    ...
    void AlbumInfo::updateAudioDBOverlay() {
        ...
        Aether::Element * body = new Aether::Element(0, 0, 700);
        Aether::TextBlock * tips = new Aether::TextBlock(40, 40, "", 24, 620);
        tips->setString(Utils::substituteTokens("Album.Information.Searching"_lang, this->metadata.name));
        tips->setColour(this->app->theme()->FG());
        body->addElement(tips);
        body->setH(tips->h() + 80);
        this->msgbox->setBody(body);
    ...
    void AlbumInfo::createFileBrowser(FBType t) {
        ...
        switch (t) {
            case FBType::Audio:
                this->browser->setHeadingText("Common.SelectAudio"_lang);
    ...
    void AlbumInfo::updateImageFromTag(const std::string & path) {
        ...
        if (tmpImage->textureWidth() == 0 || tmpImage->textureHeight() == 0) {
            this->createInfoOverlay("Common.Error.CorruptArt"_lang);
            delete tmpImage;
